#执行这个文件    ---
#  生产和测试要修改最下面两行    sh /root/mytest/cloud-test/cloud-test-user/src/main/resources/sh/buildAndRunTest.sh


export version=`sudo docker images | grep cloud-test-user | awk '{print $2}'|awk -F '-' '{print $1}' | tr -d "a-zA-Z" | awk '{print int($0)}' | sort -g | tail -1 | awk -F. '{print $1}'`
#打包
  if [ $version -lt 1000 ]
    then
    version=1100
...
#测试环境
cd /root/mytest/cloud-test/cloud-test-user/src/main/resources/sh
